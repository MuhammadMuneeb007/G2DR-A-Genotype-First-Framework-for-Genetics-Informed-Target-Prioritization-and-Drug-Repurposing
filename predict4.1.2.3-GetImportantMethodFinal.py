#!/usr/bin/env python3
"""
Method Importance Ranking — Discovery + Test Replication + Disease Enrichment
============================================================================

Ranks METHODS using:
1) DISCOVERY (Train+Validation only): no test leakage
2) TEST REPLICATION (Test only): replication of discovery signals into test
3) DISEASE ENRICHMENT (Known genes): reported separately for discovery and test
...
Evidence definition for METHOD unit:
  Evidence triplet = unique (Gene, Tissue, Database)

Outputs (under <phenotype>/GeneDifferentialExpression/Files/MethodImportance):
A) Discovery ranking (train+val only)
 - method_importance_discovery_gene_level.csv
 - method_importance_discovery_evidence_level.csv
 - method_importance_discovery_combined.csv
 - method_importance_discovery_top10_table.tex
 - method_importance_discovery_report.txt

B) Disease enrichment (separate)
 - method_disease_enrichment_discovery.csv
 - method_disease_enrichment_test.csv

C) Test replication (separate)
 - method_test_replication_gene_level.csv
 - method_test_replication_evidence_level.csv

Run:
  python predict4.4.5-GetImportantMethods_Final.py migraine

Notes:
- Includes methods with 0-signal.
- Empirical p-values via hypergeometric sampling (no SciPy).
"""
...
def write_top10_latex_discovery(df: pd.DataFrame, outpath: Path) -> None:
    ...
    lines.append("\\caption{Top methods ranked by discovery (train+validation) signal enrichment of known disease genes.}")
    lines.append("\\label{tab:method_importance_discovery}")
    lines.append("\\small")
    lines.append("\\setlength{\\tabcolsep}{4pt}")
    lines.append("\\renewcommand{\\arraystretch}{1.15}")
    lines.append("\\begin{tabular}{r p{5.5cm} r r r r}")
    lines.append("\\hline")
    lines.append("\\textbf{Rank} & \\textbf{Method} & \\textbf{$N$} & \\textbf{$k$} & \\textbf{FE} & \\textbf{Dir.Cons.} \\\\")
    lines.append("\\hline")
    for _, r in top.iterrows():
        lines.append(
            f"{int(r['Rank'])} & {latex_escape(str(r['Method']))} & "
...
def empirical_p_hypergeom_geq(rng: np.random.Generator, M: int, K: int, N: int, k_obs: int, n_perm: int) -> float:
    """
    Empirical p-value for observing >=k_obs successes when sampling N without replacement
    from a population of size M with K successes.
    """
...
def compute_gene_level_enrichment(
    ...
    out["FE_genes"] = np.where(out["expected_known_genes"] > 0,
                               out["k_known_genes"] / out["expected_known_genes"],
                               0.0)
...
def compute_evidence_level_enrichment(
    ...
    # Universe evidence rate (ALL rows in df_universe)
    ...
    # Signal evidence counts per unit (significant rows only)
    ...
    out["FE_evidence"] = np.where(out["expected_known_evidence"] > 0,
                                  out["k_known_evidence"] / out["expected_known_evidence"],
                                  0.0)
...
def compute_direction_consistency(
    ...
    out = units_df.join(unit_cons, on=unit_col, how="left").with_columns(
...
def compute_replication_gene_level(
    ...
    overlap of discovery-significant genes vs test-significant genes per unit.
    """
    units_df = pl.DataFrame({unit_col: all_units})
...
def compute_replication_evidence_level(
    ...
    overlap of discovery-significant evidence vs test-significant evidence per unit.
    Evidence = unique (Gene,Tissue,Database)
    """
    units_df = pl.DataFrame({unit_col: all_units})
...
def main():
    ...
    outdir = results_dir / "MethodImportance"
    ...
    if not has_effect and not has_l2fc:
        raise ValueError("Input file must contain either 'Effect' or 'Log2FoldChange' for effect thresholding.")
    ...
    # ALL methods (from entire table)
    all_methods = sorted(df.select(pl.col("Method").unique()).to_series().to_list())

    # effect := coalesce(Effect, Log2FoldChange)  (same as tissue)
    ...
    if M_tv == 0:
        raise ValueError("Empty discovery universe after restricting to train+val. Check Dataset labels.")
    if M_test == 0:
        raise ValueError("Empty test universe after restricting to test. Check Dataset labels.")

    # Significant rows (null-guard prevents silent all-false if effect is missing/non-numeric)
    ...
    gl_disc = compute_gene_level_enrichment(
        df_sig=df_tv_sig,
        all_units=all_methods,
        unit_col="Method",
        gene_col="Gene_norm",
        known_set_in_universe=K_tv,
        M_universe=M_tv,
        n_perm=args.n_perm,
        seed=args.seed,
    )
    ...
    el_disc, evU_tv_size, evK_tv_size = compute_evidence_level_enrichment(
        ...
        all_units=all_methods,
        unit_col="Method",
        evidence_cols=["Gene_norm", "Tissue", "Database"],
        gene_col="Gene_norm",
        known_set_in_universe=K_tv,
        n_perm=args.n_perm,
        seed=args.seed + 7,
    ...
    disc = gl_disc.merge(el_disc, on="Method", how="left").merge(dc_disc, on="Method", how="left")
    ...
    # =========================================================
    # B) DISEASE ENRICHMENT (separate outputs)
    # =========================================================
    disease_disc = disc[[
        "Rank", "Method", "N_genes", "k_known_genes", "expected_known_genes", "FE_genes", "p_emp_genes"
    ]].copy()
    ...
    # SAVE OUTPUTS
    # =========================================================
    disc.to_csv(outdir / "method_importance_discovery_combined.csv", index=False)

    disc[[
        "Rank", "Method",
        "N_genes", "k_known_genes", "expected_known_genes", "FE_genes", "p_emp_genes"
    ]].to_csv(outdir / "method_importance_discovery_gene_level.csv", index=False)

    disc[[
        "Rank", "Method",
        "N_evidence", "k_known_evidence", "expected_known_evidence", "FE_evidence", "p_emp_evidence",
        "N_tissues", "N_databases"
    ]].to_csv(outdir / "method_importance_discovery_evidence_level.csv", index=False)

    write_top10_latex_discovery(disc, outdir / "method_importance_discovery_top10_table.tex")

    disease_disc.to_csv(outdir / "method_disease_enrichment_discovery.csv", index=False)
    disease_test.to_csv(outdir / "method_disease_enrichment_test.csv", index=False)

    rep_gene.to_csv(outdir / "method_test_replication_gene_level.csv", index=False)
    rep_evd.to_csv(outdir / "method_test_replication_evidence_level.csv", index=False)

    # Report
    report_path = outdir / "method_importance_discovery_report.txt"
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("METHOD IMPORTANCE — DISCOVERY + TEST REPLICATION + DISEASE ENRICHMENT\n")
        f.write("=" * 120 + "\n")
        ...
        f.write(f"Train label: {train_label}\n")
        f.write(f"Val label:   {val_label}\n")
        f.write(f"Test label:  {test_label}\n\n")
        f.write(f"Discovery universe genes M_tv={M_tv}, known in universe K_tv={len(K_tv)}\n")
        f.write(f"Discovery evidence universe evU_tv={evU_tv_size}, known evidence evK_tv={evK_tv_size}\n")
        f.write(f"Test universe genes M_test={M_test}, known in universe K_test={len(K_test)}\n\n")
        ...
        f.write("Discovery ranking (full table):\n\n")
        f.write(disc[cols].to_string(index=False))
        f.write("\n")

    # =========================================================
    # PRINT TO SCREEN — ALL METHODS (per your request)
    # =========================================================
    print("\n✅ Completed METHOD importance.")
    print(f"📁 Output directory: {outdir}\n")

    print("============================================================")
    print("DISCOVERY RANKING — ALL METHODS (train+val only)")
    print("============================================================")
    show_disc_cols = [
        "Rank", "Method",
        "N_genes", "k_known_genes", "expected_known_genes", "FE_genes", "p_emp_genes",
    ...
    print("\n============================================================")
    print("DISEASE-BASED ENRICHMENT (DISCOVERY) — ALL METHODS")
    print("============================================================")
    print(disease_disc.to_string(index=False))

    print("\n============================================================")
    print("DISEASE-BASED ENRICHMENT (TEST) — ALL METHODS")
    print("============================================================")
    print(disease_test.to_string(index=False))

    print("\n============================================================")
    print("TEST REPLICATION (GENE-LEVEL) — ALL METHODS")
    print("============================================================")
    print(rep_gene.to_string(index=False))

    print("\n============================================================")
    print("TEST REPLICATION (EVIDENCE-LEVEL) — ALL METHODS")
    print("============================================================")
    print(rep_evd.to_string(index=False))

    print("\nFiles written:")
    print(" - method_importance_discovery_gene_level.csv")
    print(" - method_importance_discovery_evidence_level.csv")
    print(" - method_importance_discovery_combined.csv")
    print(" - method_importance_discovery_top10_table.tex")
    print(" - method_importance_discovery_report.txt")
    print(" - method_disease_enrichment_discovery.csv")
    print(" - method_disease_enrichment_test.csv")
    print(" - method_test_replication_gene_level.csv")
    print(" - method_test_replication_evidence_level.csv")
