...
TrainExpression_dir = Path + "UTMOSTTrainExpression/"
ValidationExpression_dir = Path + "UTMOSTValidationExpression/"
TestExpression_dir = Path + "UTMOSTTestExpression/"
...
def get_available_tissues():
    """
    Scan the JTI directory for available UTMOST tissue models
    ...
    # Look for UTMOST .db and .txt.gz files in the JTI directory
    try:
        tissue_models = {}  # tissue_name -> (model_path, file_type)
        
        for filename in os.listdir(jti_models_dir):
            if filename.startswith("UTMOST_"):
                tissue_name = None
                file_type = None
                
                if filename.endswith(".db"):
                    tissue_name = filename[7:-3]  # Remove "UTMOST_" and ".db"
                    file_type = "db"
                elif filename.endswith(".txt.gz"):
                    tissue_name = filename[7:-7]  # Remove "UTMOST_" and ".txt.gz"
                    file_type = "txt.gz"
        ...
        print "[INFO] Found {} UTMOST tissue models:".format(len(tissues))
...
def predict_expression_for_tissue(dosage_dir, expression_dir, dataset_name, tissue_name, tissue_model_path):
    """
    Predict gene expression for a tissue using ALL chromosomes together using UTMOST/JTI models
    ...
    Args:
        ...
        tissue_model_path: Path to tissue-specific UTMOST model file (.db or .txt.gz)
    
    Returns:
        Path to the output file or None if failed
    """
    print "[INFO] Predicting expression for {} - {} using UTMOST model (using ALL chromosomes)".format(dataset_name, tissue_name)
    ...
    try:
        # PrediXcan command for UTMOST/JTI models
        cmd = [
            "python2",
        ...
        print "[INFO] Running UTMOST PrediXcan: {}".format(' '.join(cmd))
        ...
        if os.path.exists(output_file):
            print "[INFO] UTMOST expression prediction completed: {}".format(output_file)
        ...
        print "[ERROR] Failed to predict expression for {} {} using UTMOST: {}".format(dataset_name, tissue_name, e)
...
def predict_expression_for_dataset(dosage_dir, expression_dir, dataset_name, tissues):
    """
    Predict gene expression for all UTMOST tissues in a dataset
    ...
    """
    print "[INFO] Starting UTMOST gene expression prediction for {} data".format(dataset_name)
    print "[INFO] Input directory: {}".format(dosage_dir)
    print "[INFO] Output directory: {}".format(expression_dir)
    print "[INFO] Processing {} UTMOST tissues (using ALL chromosomes together)".format(len(tissues))
    ...
    for tissue_idx, (tissue_name, tissue_model_path) in enumerate(tissues, 1):
        print "\n[INFO] --- Processing UTMOST tissue {}/{}: {} ---".format(tissue_idx, len(tissues), tissue_name)
        
        # Check if tissue model file exists
        if not os.path.exists(tissue_model_path):
            print "[WARNING] UTMOST tissue model not found: {}".format(tissue_model_path)
            continue
        ...
        if output_file and os.path.exists(output_file):
            ...
            print "[INFO] [OK] {} UTMOST {} tissue completed".format(dataset_name, tissue_name)
        else:
            print "[WARNING] {} UTMOST {} tissue failed".format(dataset_name, tissue_name)
...
def main():
    """Main function to predict gene expression for all datasets and UTMOST tissues"""
    
    # Get available UTMOST tissues
    tissues = get_available_tissues()
    if not tissues:
        print "[ERROR] No UTMOST tissue models found in: {}".format(jti_models_dir)
        sys.exit(1)
    ...
    print "[INFO] UTMOST Gene Expression Prediction Pipeline"
    print "[INFO] " + "=" * 60
    print "[INFO] Working directory: {}".format(os.getcwd())
    print "[INFO] Base path: {}".format(Path)
    print "[INFO] JTI/UTMOST models directory: {}".format(jti_models_dir)
    print "[INFO] UTMOST tissues available: {}".format(len(tissues))
    print "[INFO] Chromosomes: {}".format(", ".join(map(str, chromosomes)))
    print "[INFO] Method: Run PrediXcan ONCE per UTMOST tissue using ALL chromosomes"
    ...
    print "[INFO] Total UTMOST predictions to perform: {} datasets x {} tissues = {}".format(
    ...
    if not os.path.exists(Path):
        ...
        print "  ├── TestDosage/"
        print "  ├── UTMOSTTrainExpression/"
        print "  ├── UTMOSTValidationExpression/"
        print "  └── UTMOSTTestExpression/"
        sys.exit(1)
    ...
    if not os.path.exists(jti_models_dir):
        print "[ERROR] JTI/UTMOST models directory not found: {}".format(jti_models_dir)
        sys.exit(1)
    print "[INFO] [OK] JTI/UTMOST models directory verified"
    ...
    # Final summary
    print "\n" + "=" * 80
    print "[INFO] UTMOST Gene Expression Prediction Summary"
    print "=" * 80
    ...
    if all_successful_results:
        ...
        print "[INFO] SUCCESS! Completed {}/{} datasets using UTMOST models".format(total_datasets_completed, len(available_datasets))
        print "[INFO] Total UTMOST tissues processed: {}".format(total_tissues_completed)
        print "[INFO] Successfully processed:"
        
        for dataset_name, tissue_results in all_successful_results.items():
            print "  - {} data: {} UTMOST tissues".format(dataset_name.capitalize(), len(tissue_results))
            for tissue_name in sorted(tissue_results.keys()):
                print "    * {}".format(tissue_name)
    else:
        print "[ERROR] No datasets were successfully processed with UTMOST models!"
        sys.exit(1)
    ...
    for _, expression_dir, name in datasets:
        if os.path.exists(expression_dir):
            ...
            print "[INFO] {}: {} UTMOST expression files created".format(name.capitalize(), dataset_files)
    
    print "[INFO] Total UTMOST expression files created: {}".format(total_expression_files)
    print "[INFO] UTMOST gene expression prediction pipeline completed!"
...
if __name__ == "__main__":
    ...
    # Check if JTI/UTMOST models directory exists
    if not os.path.exists(jti_models_dir):
        print "[ERROR] JTI/UTMOST models directory not found: {}".format(jti_models_dir)
    ...
    print "[INFO] Starting UTMOST multi-tissue gene expression prediction pipeline...\n"
