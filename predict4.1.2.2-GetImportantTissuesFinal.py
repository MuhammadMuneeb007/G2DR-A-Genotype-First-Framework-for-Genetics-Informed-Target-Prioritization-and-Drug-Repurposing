#!/usr/bin/env python3
"""
Tissue Importance — Discovery Ranking + Test Replication + Disease Enrichment
============================================================================
...
Evidence definition for TISSUE unit:
  Evidence triplet = unique (Gene, Method, Database)

Outputs (under <phenotype>/GeneDifferentialExpression/Files/TissueImportance):
A) Discovery ranking (train+val only)
 - tissue_importance_discovery_gene_level.csv
 - tissue_importance_discovery_evidence_level.csv
 - tissue_importance_discovery_combined.csv
 - tissue_importance_discovery_top10_table.tex
 - tissue_importance_discovery_report.txt

B) Disease enrichment (separate)
 - tissue_disease_enrichment_discovery.csv
 - tissue_disease_enrichment_test.csv

C) Test replication (separate)
 - tissue_test_replication_gene_level.csv
 - tissue_test_replication_evidence_level.csv

Run:
  python predict4.4.8-GetImportantTissueFinal.py migraine
"""
...
# -----------------------------
# HARD-CODED thresholds (per your instruction)
# -----------------------------
FDR_THR: float = 0.10
...
def _as_set_of_evidence_tuples(x, cols: list[str]) -> set[tuple]:
    """
    Polars 'struct(...).unique()' becomes a list of dict-like rows when converted to pandas,
    but can vary. This makes it robust.
    """
    if x is None:
        return set()
    if isinstance(x, np.ndarray):
...
def compute_gene_level_enrichment(
    ...
    df_sig = df_sig.with_columns(pl.col(gene_col).is_in(sorted(known_set_in_universe)).alias("is_known"))

    stats = df_sig.group_by(unit_col).agg([
...
def compute_evidence_level_enrichment(
    ...
    """
    Evidence enrichment: compare known evidence rate to universe evidence rate.
    Universe = ALL evidence in df_universe (train+val for discovery; or test for test enrichment).
    Signal set = evidence in df_sig (significant).
    """
    units_df = pl.DataFrame({unit_col: all_units})

    df_univ = df_universe.with_columns(pl.col(gene_col).is_in(sorted(known_set_in_universe)).alias("is_known"))
    evU_size = df_univ.select(pl.struct(evidence_cols).alias("ev")).unique().height
    evK_size = df_univ.filter(pl.col("is_known")).select(pl.struct(evidence_cols).alias("ev")).unique().height

    df_sig2 = df_sig.with_columns(pl.col(gene_col).is_in(sorted(known_set_in_universe)).alias("is_known"))
    ev_stats = df_sig2.group_by(unit_col).agg([
...
def compute_direction_consistency(
    ...
    df_sig = df_sig.with_columns(
        pl.when(effect_expr > 0).then(1).when(effect_expr < 0).then(-1).otherwise(0).alias("sign")
    )
    ...
    unit_cons = gene_cons.group_by(unit_col).agg([
        ((pl.col("w") * pl.col("gene_consistency")).sum() / pl.col("w").sum()).alias("DirectionConsistency")
    ])
...
def compute_replication_gene_level(
    ...
    """
    Replication gene-level:
    - discovery significant gene set per unit
    - test significant gene set per unit
    - overlap enrichment relative to test universe
    """
    units_df = pl.DataFrame({unit_col: all_units})
...
def compute_replication_evidence_level(
    ...
    - discovery significant evidence set per unit
    - test significant evidence set per unit
    - overlap enrichment relative to test evidence universe
    """
    units_df = pl.DataFrame({unit_col: all_units})
...
def main():
    ...
    ap.add_argument("--n_perm", type=int, default=10000, help="Random trials (default: 10000)")
    ...
    outdir = results_dir / "TissueImportance"
    ...
    # IMPORTANT: include ALL tissues from full table (train+val+test)
    all_tissues = sorted(df.select(pl.col("Tissue").unique()).to_series().to_list())
    ...
    # --- Universe definitions (gene universe is per split)
    ...
    if M_tv == 0 or M_test == 0:
        raise ValueError("Empty universe after split — check Dataset labels.")

    # --- Signal filter (drop rows where effect is null; otherwise comparisons can silently yield all-false)
    df_tv_sig = df_tv.filter(
        (pl.col("FDR").cast(pl.Float64, strict=False) < FDR_THR)
        & effect_expr.is_not_null()
        & (effect_expr.abs() >= EFFECT_THR)
    )
    ...
    # A) DISCOVERY RANKING (train+val only)
    # =========================================================
    gl_disc = compute_gene_level_enrichment(
        df_tv_sig, all_tissues, "Tissue", "Gene_norm", K_tv, M_tv, args.n_perm, args.seed
    )
    el_disc, evU_tv, evK_tv = compute_evidence_level_enrichment(
        df_tv_sig, df_tv, all_tissues, "Tissue", ["Gene_norm", "Method", "Database"],
        "Gene_norm", K_tv, args.n_perm, args.seed + 7
    )
    dc_disc = compute_direction_consistency(
        df_tv_sig, all_tissues, "Tissue", "Gene_norm", effect_expr
    )

    # For context: methods/databases coverage per tissue among discovery significant calls
    extra_counts = (
        df_tv_sig.group_by("Tissue").agg([
            pl.col("Method").n_unique().alias("N_methods"),
            pl.col("Database").n_unique().alias("N_databases"),
        ])
    )
    extra_counts = pl.DataFrame({"Tissue": all_tissues}).join(extra_counts, on="Tissue", how="left").fill_null(0).to_pandas()

    disc = (
        gl_disc
        .merge(el_disc, on="Tissue", how="left")
        .merge(dc_disc, on="Tissue", how="left")
        .merge(extra_counts, on="Tissue", how="left")
    )
    ...
    # =========================================================
    # B) DISEASE-BASED ENRICHMENT (separate files)
    #   - Discovery disease enrichment = gene-level enrichment using discovery universes (K_tv/M_tv)
    #   - Test disease enrichment      = gene-level enrichment using test universes (K_test/M_test)
    # =========================================================
    disease_disc = disc[[
        "Rank", "Tissue", "N_genes", "k_known_genes", "expected_known_genes", "FE_genes", "p_emp_genes"
    ]].copy()

    gl_test = compute_gene_level_enrichment(
        df_test_sig, all_tissues, "Tissue", "Gene_norm", K_test, M_test, args.n_perm, args.seed + 101
    )
    ...
    # =========================================================
    # C) TEST REPLICATION (separate files)
    # =========================================================
    rep_gene = compute_replication_gene_level(
        df_tv_sig, df_test_sig, df_test, all_tissues, "Tissue", "Gene_norm", args.n_perm, args.seed + 202
    ).sort_values(["FE_replication", "p_emp_replication"], ascending=[False, True]).reset_index(drop=True)

    rep_evd = compute_replication_evidence_level(
        df_tv_sig, df_test_sig, df_test, all_tissues, "Tissue", ["Gene_norm", "Method", "Database"],
        args.n_perm, args.seed + 303
    ).sort_values(["FE_replication", "p_emp_replication"], ascending=[False, True]).reset_index(drop=True)

    # =========================================================
    # SAVE OUTPUTS
    # =========================================================
    disc.to_csv(outdir / "tissue_importance_discovery_combined.csv", index=False)

    disc[["Rank", "Tissue", "N_genes", "k_known_genes", "expected_known_genes", "FE_genes", "p_emp_genes"]].to_csv(
        outdir / "tissue_importance_discovery_gene_level.csv", index=False
    )
    disc[[
        "Rank", "Tissue",
        "N_evidence", "k_known_evidence", "expected_known_evidence", "FE_evidence", "p_emp_evidence",
        "N_methods", "N_databases"
    ]].to_csv(outdir / "tissue_importance_discovery_evidence_level.csv", index=False)
 

    disease_disc.to_csv(outdir / "tissue_disease_enrichment_discovery.csv", index=False)
    disease_test.to_csv(outdir / "tissue_disease_enrichment_test.csv", index=False)

    rep_gene.to_csv(outdir / "tissue_test_replication_gene_level.csv", index=False)
    rep_evd.to_csv(outdir / "tissue_test_replication_evidence_level.csv", index=False)

    report_path = outdir / "tissue_importance_discovery_report.txt"
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("TISSUE IMPORTANCE — DISCOVERY + TEST REPLICATION + DISEASE ENRICHMENT\n")
        f.write("=" * 110 + "\n")
        f.write(f"Phenotype: {phenotype}\n")
        f.write(f"Input: {volcano_file}\n")
        f.write(f"Known gene source: {known_src}\n\n")
        f.write(f"Thresholds: FDR<{FDR_THR}, |effect|>={EFFECT_THR} (effect := coalesce(Effect, Log2FoldChange))\n\n")
        f.write(f"Discovery gene universe M_tv={M_tv}, known in universe K_tv={len(K_tv)}\n")
        f.write(f"Discovery evidence universe evU_tv={evU_tv}, known evidence evK_tv={evK_tv}\n")
        f.write(f"Test gene universe M_test={M_test}, known in universe K_test={len(K_test)}\n\n")
        f.write("Discovery ranking (first 100 rows):\n")
        f.write(disc.head(100).to_string(index=False))
        f.write("\n")

    # =========================================================
    # PRINT TO SCREEN (ALL tissues, per your request)
    # =========================================================
    print("\n✅ Completed TISSUE importance.")
    print(f"📁 Output directory: {outdir}\n")

    print("============================================================")
    print("DISCOVERY RANKING — ALL TISSUES (train+val only)")
    print("============================================================")
    show_disc_cols = [
        "Rank", "Tissue",
    ...
    print(disc[show_disc_cols].to_string(index=False))

    print("\n============================================================")
    print("DISEASE-BASED ENRICHMENT (DISCOVERY) — ALL TISSUES")
    print("============================================================")
    print(disease_disc.to_string(index=False))

    print("\n============================================================")
    print("DISEASE-BASED ENRICHMENT (TEST) — ALL TISSUES")
    print("============================================================")
    print(disease_test.to_string(index=False))

    print("\n============================================================")
    print("TEST REPLICATION (GENE-LEVEL) — ALL TISSUES")
    print("============================================================")
    print(rep_gene.to_string(index=False))

    print("\n============================================================")
    print("TEST REPLICATION (EVIDENCE-LEVEL) — ALL TISSUES")
    print("============================================================")
    print(rep_evd.to_string(index=False))

    print("\nFiles written:")
    print(" - tissue_importance_discovery_gene_level.csv")
    print(" - tissue_importance_discovery_evidence_level.csv")
    print(" - tissue_importance_discovery_combined.csv")
    print(" - tissue_importance_discovery_top10_table.tex")
    print(" - tissue_importance_discovery_report.txt")
    print(" - tissue_disease_enrichment_discovery.csv")
    print(" - tissue_disease_enrichment_test.csv")
    print(" - tissue_test_replication_gene_level.csv")
    print(" - tissue_test_replication_evidence_level.csv")
