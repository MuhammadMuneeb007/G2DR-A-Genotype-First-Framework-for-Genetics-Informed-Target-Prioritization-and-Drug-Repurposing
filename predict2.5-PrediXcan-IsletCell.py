...
TrainExpression_dir = Path + "isletcellTrainExpression/"
ValidationExpression_dir = Path + "isletcellValidationExpression/"
TestExpression_dir = Path + "isletcellTestExpression/"

# Gene expression prediction configuration
chromosomes = range(1, 23)  # Chromosomes 1-22
isletcell_models_dir = "/data/ascher02/uqmmune1/ANNOVAR/PrediXcan/isletcells"

def get_available_tissues():
    """
    Scan the islet cell directory for available PrediXcan tissue models
    ...
    if not os.path.exists(isletcell_models_dir):
        print "[ERROR] Islet cell models directory not found: {}".format(isletcell_models_dir)
        return tissues
    
    # Look for .db files in the islet cell directory
    try:
        tissue_models = {}  # tissue_name -> model_path
        
        for filename in os.listdir(isletcell_models_dir):
            if filename.endswith(".db"):
                # Extract tissue name by removing .db extension
                tissue_name = filename[:-3]  # Remove ".db"
                model_path = os.path.join(isletcell_models_dir, filename)
                tissue_models[tissue_name] = model_path
        ...
        print "[INFO] Found {} islet cell PrediXcan models:".format(len(tissues))
        for tissue_name, model_path in tissues:
            print "  - {} ({})".format(tissue_name, os.path.basename(model_path))
        
    except Exception as e:
        print "[ERROR] Failed to scan islet cell directory: {}".format(e)
...
def predict_expression_for_tissue(dosage_dir, expression_dir, dataset_name, tissue_name, tissue_model_path):
    """
    Predict gene expression for a tissue using ALL chromosomes together using islet cell PrediXcan models
    ...
    Args:
        ...
        tissue_model_path: Path to tissue-specific islet cell PrediXcan model file (.db)
    
    Returns:
        Path to the output file or None if failed
    """
    print "[INFO] Predicting expression for {} - {} using islet cell PrediXcan model (using ALL chromosomes)".format(dataset_name, tissue_name)
    ...
    # Define output file path with islet cell naming convention
    output_file = os.path.join(tissue_expression_dir, "predicted_expression_isletcell_{}_{}_merged.txt".format(dataset_name, tissue_name))
    ...
    try:
        # PrediXcan command for islet cell models
        cmd = [
            "python2",
        ...
        print "[INFO] Running islet cell PrediXcan: {}".format(' '.join(cmd))
        
        # Run with progress indication
        with tqdm(desc="Processing {} {}".format(dataset_name, tissue_name[:20]), unit="step") as pbar:
            pbar.set_description("Processing isletcell {} {}".format(dataset_name, tissue_name[:20]))
            ...
            while True:
                ...
                if output:
                    ...
                    if len(output_str) > 0:
                        ...
                        pbar.set_description("isletcell {} {}: {}".format(dataset_name, tissue_name[:15], display_output))
                    pbar.update(1)
            ...
            pbar.set_description("[OK] isletcell {} {} completed".format(dataset_name, tissue_name[:20]))
        
        if os.path.exists(output_file):
            print "[INFO] Islet cell expression prediction completed: {}".format(output_file)
        ...
        print "[ERROR] Failed to predict expression for {} {} using islet cell model: {}".format(dataset_name, tissue_name, e)
...
def predict_expression_for_dataset(dosage_dir, expression_dir, dataset_name, tissues):
    """
    Predict gene expression for all islet cell tissues in a dataset
    ...
    """
    print "[INFO] Starting islet cell gene expression prediction for {} data".format(dataset_name)
    print "[INFO] Input directory: {}".format(dosage_dir)
    print "[INFO] Output directory: {}".format(expression_dir)
    print "[INFO] Processing {} islet cell tissues (using ALL chromosomes together)".format(len(tissues))
    ...
    for tissue_idx, (tissue_name, tissue_model_path) in enumerate(tissues, 1):
        print "\n[INFO] --- Processing islet cell tissue {}/{}: {} ---".format(tissue_idx, len(tissues), tissue_name)
        
        # Check if tissue model file exists
        if not os.path.exists(tissue_model_path):
            print "[WARNING] Islet cell tissue model not found: {}".format(tissue_model_path)
            continue
        ...
        if output_file and os.path.exists(output_file):
            ...
            print "[INFO] [OK] {} islet cell {} tissue completed".format(dataset_name, tissue_name)
        else:
            print "[WARNING] {} islet cell {} tissue failed".format(dataset_name, tissue_name)
...
def main():
    """Main function to predict gene expression for all datasets and islet cell tissues"""
    
    # Get available islet cell tissues
    tissues = get_available_tissues()
    if not tissues:
        print "[ERROR] No islet cell tissue models found in: {}".format(isletcell_models_dir)
        sys.exit(1)
    ...
    print "[INFO] Pancreatic Islet Cell Gene Expression Prediction Pipeline"
    print "[INFO] " + "=" * 60
    print "[INFO] Working directory: {}".format(os.getcwd())
    print "[INFO] Base path: {}".format(Path)
    print "[INFO] Islet cell models directory: {}".format(isletcell_models_dir)
    print "[INFO] Islet cell tissues available: {}".format(len(tissues))
    print "[INFO] Chromosomes: {}".format(", ".join(map(str, chromosomes)))
    print "[INFO] Method: Run PrediXcan ONCE per islet cell tissue using ALL chromosomes"
    ...
    print "[INFO] Total islet cell predictions to perform: {} datasets x {} tissues = {}".format(
    ...
    if not os.path.exists(Path):
        ...
        print "  ├── TestDosage/"
        print "  ├── isletcellTrainExpression/"
        print "  ├── isletcellValidationExpression/"
        print "  └── isletcellTestExpression/"
        sys.exit(1)
    
    print "[INFO] [OK] Base path exists: {}".format(Path)
    
    # Check if islet cell models directory exists
    if not os.path.exists(isletcell_models_dir):
        print "[ERROR] Islet cell models directory not found: {}".format(isletcell_models_dir)
        sys.exit(1)
    print "[INFO] [OK] Islet cell models directory verified"
    ...
    # Final summary
    print "\n" + "=" * 80
    print "[INFO] Pancreatic Islet Cell Gene Expression Prediction Summary"
    print "=" * 80
    ...
    if all_successful_results:
        ...
        print "[INFO] SUCCESS! Completed {}/{} datasets using islet cell models".format(total_datasets_completed, len(available_datasets))
        print "[INFO] Total islet cell tissues processed: {}".format(total_tissues_completed)
        print "[INFO] Successfully processed:"
        
        for dataset_name, tissue_results in all_successful_results.items():
            print "  - {} data: {} islet cell tissues".format(dataset_name.capitalize(), len(tissue_results))
            for tissue_name in sorted(tissue_results.keys()):
                print "    * {}".format(tissue_name)
    else:
        print "[ERROR] No datasets were successfully processed with islet cell models!"
        sys.exit(1)
    ...
    for _, expression_dir, name in datasets:
        if os.path.exists(expression_dir):
            ...
            print "[INFO] {}: {} islet cell expression files created".format(name.capitalize(), dataset_files)
    
    print "[INFO] Total islet cell expression files created: {}".format(total_expression_files)
    print "[INFO] Pancreatic islet cell gene expression prediction pipeline completed!"
    ...
    for dataset_name in all_successful_results.keys():
        ...
        for tissue_name in sorted(all_successful_results[dataset_name].keys()):
            print "  ├── {}/".format(tissue_name)
            print "  │   └── predicted_expression_isletcell_{}_{}_merged.csv".format(dataset_name, tissue_name)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(isletcell_models_dir):
        print "[ERROR] Islet cell models directory not found: {}".format(isletcell_models_dir)
        print "[ERROR] Please check the path: {}".format(isletcell_models_dir)
        sys.exit(1)
    
    print "[INFO] [OK] All dependencies verified"
    print "[INFO] Starting pancreatic islet cell multi-tissue gene expression prediction pipeline...\n"
