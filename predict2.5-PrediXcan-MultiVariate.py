...
TrainExpression_dir = Path + "multivariateTrainExpression/"
ValidationExpression_dir = Path + "multivariateValidationExpression/"
TestExpression_dir = Path + "multivariateTestExpression/"

# Gene expression prediction configuration
chromosomes = range(1, 23)  # Chromosomes 1-22
multivariate_models_dir = "/data/ascher02/uqmmune1/ANNOVAR/PrediXcan/multivariate"

def get_available_tissues():
    """
    Scan the multivariate directory for available PrediXcan tissue models
    ...
    if not os.path.exists(multivariate_models_dir):
        print "[ERROR] Multivariate models directory not found: {}".format(multivariate_models_dir)
        return tissues
    
    # Look for .db files in the multivariate directory
    try:
        tissue_models = {}  # tissue_name -> model_path
        
        for filename in os.listdir(multivariate_models_dir):
            if filename.endswith(".db"):
                # Extract tissue name by removing .db extension
                tissue_name = filename[:-3]  # Remove ".db"
                model_path = os.path.join(multivariate_models_dir, filename)
                tissue_models[tissue_name] = model_path
        
        # Convert to list format
        for tissue_name, model_path in tissue_models.items():
            tissues.append((tissue_name, model_path))
        
        tissues.sort()  # Sort alphabetically
        print "[INFO] Found {} multivariate PrediXcan models:".format(len(tissues))
        
        # Group by cell type and population for better display
        cell_types = {}
        for tissue_name, model_path in tissues:
            parts = tissue_name.split('_')
            if len(parts) >= 2:
                cell_type = parts[0]  # Mono, PBMC, Tcell
                population = parts[1]  # AFA, EUR, HIS, CHN
                method = '_'.join(parts[2:])  # EN_baseline, mashr_baseline, etc.
                
                if cell_type not in cell_types:
                    cell_types[cell_type] = {}
                if population not in cell_types[cell_type]:
                    cell_types[cell_type][population] = []
                cell_types[cell_type][population].append(method)
        
        # Display organized summary
        for cell_type in sorted(cell_types.keys()):
            print "  [{}] - {} populations:".format(cell_type, len(cell_types[cell_type]))
            for population in sorted(cell_types[cell_type].keys()):
                methods = cell_types[cell_type][population]
                print "    - {}: {} methods ({})".format(population, len(methods), ', '.join(sorted(methods)))
        ...
        print "[ERROR] Failed to scan multivariate directory: {}".format(e)
...
def predict_expression_for_tissue(dosage_dir, expression_dir, dataset_name, tissue_name, tissue_model_path):
    """
    Predict gene expression for a tissue using ALL chromosomes together using multivariate PrediXcan models
    ...
    Args:
        ...
        tissue_model_path: Path to tissue-specific multivariate PrediXcan model file (.db)
    
    Returns:
        Path to the output file or None if failed
    """
    print "[INFO] Predicting expression for {} - {} using multivariate PrediXcan model (using ALL chromosomes)".format(dataset_name, tissue_name)
    ...
    # Define output file path with multivariate naming convention
    output_file = os.path.join(tissue_expression_dir, "predicted_expression_multivariate_{}_{}_merged.txt".format(dataset_name, tissue_name))
    ...
    try:
        # PrediXcan command for multivariate models
        cmd = [
            "python2",
        ...
        print "[INFO] Running multivariate PrediXcan: {}".format(' '.join(cmd))
        
        # Run with progress indication
        with tqdm(desc="Processing {} {}".format(dataset_name, tissue_name[:20]), unit="step") as pbar:
            pbar.set_description("Processing multivariate {} {}".format(dataset_name, tissue_name[:20]))
            ...
            while True:
                ...
                if output:
                    ...
                    if len(output_str) > 0:
                        ...
                        pbar.set_description("multivariate {} {}: {}".format(dataset_name, tissue_name[:15], display_output))
                    pbar.update(1)
            ...
            pbar.set_description("[OK] multivariate {} {} completed".format(dataset_name, tissue_name[:20]))
        
        if os.path.exists(output_file):
            print "[INFO] Multivariate expression prediction completed: {}".format(output_file)
        ...
        print "[ERROR] Failed to predict expression for {} {} using multivariate model: {}".format(dataset_name, tissue_name, e)
...
def predict_expression_for_dataset(dosage_dir, expression_dir, dataset_name, tissues):
    """
    Predict gene expression for all multivariate tissues in a dataset
    ...
    """
    print "[INFO] Starting multivariate gene expression prediction for {} data".format(dataset_name)
    print "[INFO] Input directory: {}".format(dosage_dir)
    print "[INFO] Output directory: {}".format(expression_dir)
    print "[INFO] Processing {} multivariate tissues (using ALL chromosomes together)".format(len(tissues))
    ...
    for tissue_idx, (tissue_name, tissue_model_path) in enumerate(tissues, 1):
        print "\n[INFO] --- Processing multivariate tissue {}/{}: {} ---".format(tissue_idx, len(tissues), tissue_name)
        
        # Check if tissue model file exists
        if not os.path.exists(tissue_model_path):
            print "[WARNING] Multivariate tissue model not found: {}".format(tissue_model_path)
            continue
        ...
        if output_file and os.path.exists(output_file):
            ...
            print "[INFO] [OK] {} multivariate {} tissue completed".format(dataset_name, tissue_name)
        else:
            print "[WARNING] {} multivariate {} tissue failed".format(dataset_name, tissue_name)
...
def main():
    """Main function to predict gene expression for all datasets and multivariate tissues"""
    
    # Get available multivariate tissues
    tissues = get_available_tissues()
    if not tissues:
        print "[ERROR] No multivariate tissue models found in: {}".format(multivariate_models_dir)
        sys.exit(1)
    ...
    print "[INFO] Multivariate Gene Expression Prediction Pipeline"
    print "[INFO] " + "=" * 60
    print "[INFO] Working directory: {}".format(os.getcwd())
    print "[INFO] Base path: {}".format(Path)
    print "[INFO] Multivariate models directory: {}".format(multivariate_models_dir)
    print "[INFO] Multivariate tissues available: {}".format(len(tissues))
    print "[INFO] Chromosomes: {}".format(", ".join(map(str, chromosomes)))
    print "[INFO] Method: Run PrediXcan ONCE per multivariate tissue using ALL chromosomes"
    ...
    print "[INFO] Total multivariate predictions to perform: {} datasets x {} tissues = {}".format(
    ...
    if not os.path.exists(Path):
        ...
        print "  ├── TestDosage/"
        print "  ├── multivariateTrainExpression/"
        print "  ├── multivariateValidationExpression/"
        print "  └── multivariateTestExpression/"
        sys.exit(1)
    
    print "[INFO] [OK] Base path exists: {}".format(Path)
    
    # Check if multivariate models directory exists
    if not os.path.exists(multivariate_models_dir):
        print "[ERROR] Multivariate models directory not found: {}".format(multivariate_models_dir)
        sys.exit(1)
    print "[INFO] [OK] Multivariate models directory verified"
    ...
    # Final summary
    print "\n" + "=" * 80
    print "[INFO] Multivariate Gene Expression Prediction Summary"
    print "=" * 80
    ...
    if all_successful_results:
        ...
        print "[INFO] SUCCESS! Completed {}/{} datasets using multivariate models".format(total_datasets_completed, len(available_datasets))
        print "[INFO] Total multivariate tissues processed: {}".format(total_tissues_completed)
        print "[INFO] Successfully processed:"
        
        for dataset_name, tissue_results in all_successful_results.items():
            print "  - {} data: {} multivariate tissues".format(dataset_name.capitalize(), len(tissue_results))
            
            # Group results by cell type for better display
            cell_type_groups = {}
            for tissue_name in sorted(tissue_results.keys()):
                ...
                if len(parts) >= 1:
                    ...
                    cell_type_groups[cell_type].append(tissue_name)
            
            for cell_type in sorted(cell_type_groups.keys()):
                print "    [{}]: {} models".format(cell_type, len(cell_type_groups[cell_type]))
                for tissue_name in sorted(cell_type_groups[cell_type]):
                    print "      * {}".format(tissue_name)
    else:
        print "[ERROR] No datasets were successfully processed with multivariate models!"
        sys.exit(1)
    ...
    for _, expression_dir, name in datasets:
        if os.path.exists(expression_dir):
            ...
            print "[INFO] {}: {} multivariate expression files created".format(name.capitalize(), dataset_files)
    
    print "[INFO] Total multivariate expression files created: {}".format(total_expression_files)
    print "[INFO] Multivariate gene expression prediction pipeline completed!"
    ...
    for dataset_name in all_successful_results.keys():
        ...
        for tissue_name in sorted(all_successful_results[dataset_name].keys()):
            print "  ├── {}/".format(tissue_name)
            print "  │   └── predicted_expression_multivariate_{}_{}_merged.csv".format(dataset_name, tissue_name)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(multivariate_models_dir):
        print "[ERROR] Multivariate models directory not found: {}".format(multivariate_models_dir)
        print "[ERROR] Please check the path: {}".format(multivariate_models_dir)
        sys.exit(1)
    
    print "[INFO] [OK] All dependencies verified"
    print "[INFO] Starting multivariate multi-tissue gene expression prediction pipeline...\n"
