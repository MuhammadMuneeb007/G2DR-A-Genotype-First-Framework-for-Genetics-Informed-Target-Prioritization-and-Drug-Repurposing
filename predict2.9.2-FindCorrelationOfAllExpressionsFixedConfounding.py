#!/usr/bin/env python3
"""
Simple correlation summary (EQUAL WEIGHTS IN r-SPACE)
====================================================
...
- "Sum" would break correlation bounds; we use mean to keep r in [-1, 1].
- CIs/p-values: folds are not truly independent replicates, so we report
  descriptive uncertainty (mean ± 1.96*SE across folds) and do NOT claim
  hypothesis-testing validity.

Outputs (saved under <phenotype>/):
  - correlation_<split>_equalweights.png
  - correlation_results_equalweights.csv
  - correlation_table_equalweights.tex
"""
...
if len(sys.argv) != 2:
    print("Usage: python3 correlation_equalweights.py <phenotype>")
    sys.exit(1)
...
# If you are using confounding-adjusted files for FUSION:
FILE_NAMES_FUSION = {
...
# For non-FUSION models, we pick the first CSV containing "_fixed" (case-insensitive).
# If you want non-fixed files, change "_fixed" to "" in find_file().
REQUIRE_FIXED = True
...
def find_file(tissue_dir: str, model: str, split: str):
    ...
    if REQUIRE_FIXED:
        csvs = [f for f in csvs if "_fixed" in f.lower()]
    else:
    ...
    # prefer shortest name (often the intended one)
...
def analyze_split_equalweights(phenotype_dir: str, folds, split: str):
    ...
    # Store fold-level rs for each pair (so split r is mean across folds)
    fold_rs = {f"{a}_vs_{b}": [] for a, b in pairs}

    # also store counts for transparency
    ...
    for fold in folds:
        ...
        for tissue in tissues:
            ...
            # compute tissue r for each pair (if both present)
            for m1, m2 in pairs:
    ...
    for pk, rs in fold_rs.items():
        ...
        # descriptive uncertainty across folds (NOT a strict inferential CI)
        if len(rs_arr) > 1:
        ...
        # keep within [-1,1] just in case of numeric jitter
...
def plot_matrix(results, models, split, phenotype_dir):
    ...
    ax.set_title(f"{split.capitalize()} - {phenotype_dir} (equal weights)", fontsize=14, weight="bold")
    plt.tight_layout()

    os.makedirs(phenotype_dir, exist_ok=True)
    out = os.path.join(phenotype_dir, f"correlation_{split}_equalweights.png")
...
def save_csv(all_results, phenotype_dir):
    ...
    os.makedirs(phenotype_dir, exist_ok=True)
    out = os.path.join(phenotype_dir, "correlation_results_equalweights.csv")
...
def save_latex(all_results, phenotype_dir):
    os.makedirs(phenotype_dir, exist_ok=True)
    out = os.path.join(phenotype_dir, "correlation_table_equalweights.tex")

    # use train keys as reference ordering
    pairs = sorted(all_results.get("train", {}).keys())

    with open(out, "w") as f:
        f.write("\\begin{table*}[htbp]\n\\centering\n")
        f.write(f"\\caption{{Cross-model correlations ({phenotype_dir}, equal weights in $r$-space). ")
...
def print_results(all_results):
    print("\n" + "=" * 90)
    print("RESULTS (equal weights in r-space)")
    print("=" * 90)
...
print("\n" + "=" * 70)
print("CORRELATION ANALYSIS (EQUAL WEIGHTS IN r-SPACE)")
print("=" * 70)
print("Logic:")
...
print(f"\nPhenotype: {phenotype}")
print(f"Folds: {folds}")
print(f"Using confounding-adjusted files: {REQUIRE_FIXED} (and FUSION *_fixed names)\n")
...
for split in ["train", "validation", "test"]:
    ...
    models_ref = models_ref or models
