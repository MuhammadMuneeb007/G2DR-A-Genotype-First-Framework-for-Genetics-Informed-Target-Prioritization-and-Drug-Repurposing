...
TrainExpression_dir = Path + "utmost2TrainExpression/"
ValidationExpression_dir = Path + "utmost2ValidationExpression/"
TestExpression_dir = Path + "utmost2TestExpression/"

# Gene expression prediction configuration
chromosomes = range(1, 23)  # Chromosomes 1-22
utmost_models_dir = "/data/ascher02/uqmmune1/ANNOVAR/UTMOST/utmost_data"

def get_available_tissues():
    """
    Scan the UTMOST directory for available CTIMP tissue models
    ...
    if not os.path.exists(utmost_models_dir):
        print "[ERROR] UTMOST models directory not found: {}".format(utmost_models_dir)
        return tissues
    
    # Look for CTIMP .db and .txt.gz files in the UTMOST directory
    try:
        tissue_models = {}  # tissue_name -> (model_path, file_type)
        
        for filename in os.listdir(utmost_models_dir):
            if filename.startswith("ctimp_"):
                tissue_name = None
                file_type = None
                
                if filename.endswith(".db"):
                    tissue_name = filename[6:-3]  # Remove "ctimp_" and ".db"
                    file_type = "db"
                elif filename.endswith(".txt.gz"):
                    tissue_name = filename[6:-7]  # Remove "ctimp_" and ".txt.gz"
                    file_type = "txt.gz"
                
                if tissue_name:
                    model_path = os.path.join(utmost_models_dir, filename)
        ...
        print "[INFO] Found {} UTMOST CTIMP tissue models:".format(len(tissues))
        ...
        print "[ERROR] Failed to scan UTMOST directory: {}".format(e)
...
def predict_expression_for_tissue(dosage_dir, expression_dir, dataset_name, tissue_name, tissue_model_path):
    """
    Predict gene expression for a tissue using ALL chromosomes together using UTMOST CTIMP models
    ...
    Args:
        ...
        tissue_model_path: Path to tissue-specific UTMOST CTIMP model file (.db or .txt.gz)
    
    Returns:
        Path to the output file or None if failed
    """
    print "[INFO] Predicting expression for {} - {} using UTMOST CTIMP model (using ALL chromosomes)".format(dataset_name, tissue_name)
    ...
    # Define output file path with UTMOST naming convention
    output_file = os.path.join(tissue_expression_dir, "predicted_expression_UTMOST_{}_{}_merged.txt".format(dataset_name, tissue_name))
    ...
    try:
        # PrediXcan command for UTMOST CTIMP models
        cmd = [
            "python2",
        ...
        print "[INFO] Running UTMOST CTIMP PrediXcan: {}".format(' '.join(cmd))
        
        # Run with progress indication
        with tqdm(desc="Processing {} {}".format(dataset_name, tissue_name[:20]), unit="step") as pbar:
            pbar.set_description("Processing UTMOST {} {}".format(dataset_name, tissue_name[:20]))
            ...
            while True:
                ...
                if output:
                    ...
                    if len(output_str) > 0:
                        ...
                        pbar.set_description("UTMOST {} {}: {}".format(dataset_name, tissue_name[:15], display_output))
                    pbar.update(1)
            ...
            pbar.set_description("[OK] UTMOST {} {} completed".format(dataset_name, tissue_name[:20]))
        
        if os.path.exists(output_file):
            print "[INFO] UTMOST CTIMP expression prediction completed: {}".format(output_file)
        ...
        print "[ERROR] Failed to predict expression for {} {} using UTMOST CTIMP: {}".format(dataset_name, tissue_name, e)
...
def predict_expression_for_dataset(dosage_dir, expression_dir, dataset_name, tissues):
    """
    Predict gene expression for all UTMOST CTIMP tissues in a dataset
    ...
    """
    print "[INFO] Starting UTMOST CTIMP gene expression prediction for {} data".format(dataset_name)
    print "[INFO] Input directory: {}".format(dosage_dir)
    print "[INFO] Output directory: {}".format(expression_dir)
    print "[INFO] Processing {} UTMOST CTIMP tissues (using ALL chromosomes together)".format(len(tissues))
    ...
    for tissue_idx, (tissue_name, tissue_model_path) in enumerate(tissues, 1):
        print "\n[INFO] --- Processing UTMOST CTIMP tissue {}/{}: {} ---".format(tissue_idx, len(tissues), tissue_name)
        
        # Check if tissue model file exists
        if not os.path.exists(tissue_model_path):
            print "[WARNING] UTMOST CTIMP tissue model not found: {}".format(tissue_model_path)
            continue
        ...
        if output_file and os.path.exists(output_file):
            ...
            print "[INFO] [OK] {} UTMOST CTIMP {} tissue completed".format(dataset_name, tissue_name)
        else:
            print "[WARNING] {} UTMOST CTIMP {} tissue failed".format(dataset_name, tissue_name)
...
def main():
    """Main function to predict gene expression for all datasets and UTMOST CTIMP tissues"""
    
    # Get available UTMOST CTIMP tissues
    tissues = get_available_tissues()
    if not tissues:
        print "[ERROR] No UTMOST CTIMP tissue models found in: {}".format(utmost_models_dir)
        sys.exit(1)
    ...
    print "[INFO] UTMOST CTIMP Gene Expression Prediction Pipeline"
    print "[INFO] " + "=" * 60
    print "[INFO] Working directory: {}".format(os.getcwd())
    print "[INFO] Base path: {}".format(Path)
    print "[INFO] UTMOST CTIMP models directory: {}".format(utmost_models_dir)
    print "[INFO] UTMOST CTIMP tissues available: {}".format(len(tissues))
    print "[INFO] Chromosomes: {}".format(", ".join(map(str, chromosomes)))
    print "[INFO] Method: Run PrediXcan ONCE per UTMOST CTIMP tissue using ALL chromosomes"
    ...
    print "[INFO] Total UTMOST CTIMP predictions to perform: {} datasets x {} tissues = {}".format(
    ...
    if not os.path.exists(Path):
        ...
        print "  ├── TestDosage/"
        print "  ├── utmost2TrainExpression/"
        print "  ├── utmost2ValidationExpression/"
        print "  └── utmost2TestExpression/"
        sys.exit(1)
    
    print "[INFO] [OK] Base path exists: {}".format(Path)
    
    # Check if UTMOST CTIMP models directory exists
    if not os.path.exists(utmost_models_dir):
        print "[ERROR] UTMOST CTIMP models directory not found: {}".format(utmost_models_dir)
        sys.exit(1)
    print "[INFO] [OK] UTMOST CTIMP models directory verified"
    ...
    # Final summary
    print "\n" + "=" * 80
    print "[INFO] UTMOST CTIMP Gene Expression Prediction Summary"
    print "=" * 80
    ...
    if all_successful_results:
        ...
        print "[INFO] SUCCESS! Completed {}/{} datasets using UTMOST CTIMP models".format(total_datasets_completed, len(available_datasets))
        print "[INFO] Total UTMOST CTIMP tissues processed: {}".format(total_tissues_completed)
        print "[INFO] Successfully processed:"
        
        for dataset_name, tissue_results in all_successful_results.items():
            print "  - {} data: {} UTMOST CTIMP tissues".format(dataset_name.capitalize(), len(tissue_results))
            for tissue_name in sorted(tissue_results.keys()):
                print "    * {}".format(tissue_name)
    else:
        print "[ERROR] No datasets were successfully processed with UTMOST CTIMP models!"
        sys.exit(1)
    ...
    for _, expression_dir, name in datasets:
        if os.path.exists(expression_dir):
            ...
            print "[INFO] {}: {} UTMOST CTIMP expression files created".format(name.capitalize(), dataset_files)
    
    print "[INFO] Total UTMOST CTIMP expression files created: {}".format(total_expression_files)
    print "[INFO] UTMOST CTIMP gene expression prediction pipeline completed!"
    ...
    for dataset_name in all_successful_results.keys():
        ...
        for tissue_name in sorted(all_successful_results[dataset_name].keys()):
            print "  ├── {}/".format(tissue_name)
            print "  │   └── predicted_expression_UTMOST_{}_{}_merged.csv".format(dataset_name, tissue_name)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(utmost_models_dir):
        print "[ERROR] UTMOST CTIMP models directory not found: {}".format(utmost_models_dir)
        print "[ERROR] Please check the path: {}".format(utmost_models_dir)
        sys.exit(1)
    
    print "[INFO] [OK] All dependencies verified"
    print "[INFO] Starting UTMOST CTIMP multi-tissue gene expression prediction pipeline...\n"
